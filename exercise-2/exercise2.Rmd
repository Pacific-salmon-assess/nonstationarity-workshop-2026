...
mse.prod.stable=salmon_sim.tv_hcr(log.a0=log.a0,p.change=p.change,smax0=smax0,sigma=sigma,N=N,par='a',form='linear',hcr.form='stable',hcr.par=NULL,assess.freq=assess.freq,eg.scalar=eg.scalar,upper.tar.scalar=upper.tar.scalar,U.scalar=U.scalar,U.min=U.min)

mse.prod.ns=salmon_sim.tv_hcr(log.a0=log.a0,p.change=p.change,smax0=smax0,sigma=sigma,N=N,par='a',form='linear',hcr.form='rw',hcr.par='a',assess.freq=assess.freq,eg.scalar=eg.scalar,upper.tar.scalar=upper.tar.scalar,U.scalar=U.scalar,U.min=U.min)
...
points(mse.prod.stable$umsy.true[tp[1]]~mse.prod.stable$smsy.true[tp[1]],pch=21,cex=2,bg='goldenrod')
points(mse.prod.stable$umsy.true[tp[2]]~mse.prod.stable$smsy.true[tp[2]],pch=21,cex=2,bg='darkred')
points(mse.prod.stable$umsy.true[tp[3]]~mse.prod.stable$smsy.true[tp[3]],pch=21,cex=2,bg='navy')
...
points(c(mean.catch.st,mean.spn.st)~c(0.9,1.9),pch=21,bg='navy',cex=2)
lines(c(mean.catch.ns,mean.spn.ns)~c(1.1,2.1),lwd=2,col='darkred')
points(c(mean.catch.ns,mean.spn.ns)~c(1.1,2.1),pch=21,bg='darkred',cex=2)
...
```{r, closed loop sims segment, fig.width=6,fig.height=10,warning=FALSE}
log.a0=1.5 #initial productivity
p.change=-0.75 #proprtional change in time-varying parameter, -0.75 = -75% (on the log-scale), 0.5 = +50%
smax0=5000 #initial smax
sigma=0.6 #most stocks range from ~0.3 to 1.5
...
points(c(median(m.catch.st),median(m.spn.st))~c(0.9,1.9),pch=21,bg='navy',cex=2)
lines(c(median(m.catch.ns),median(m.spn.ns))~c(1.1,2.1),lwd=2,col='darkred')
points(c(median(m.catch.ns),median(m.spn.ns))~c(1.1,2.1),pch=21,bg='darkred',cex=2)
mtext(side=1,'Catch',at=1,line=1)
mtext(side=1,'Spawners',at=2,line=1)
text(x=par('usr')[1]*1.4,y=par('usr')[4]-(par('usr')[4]-par('usr')[3])*0.15,'Stable',col='navy')
text(x=par('usr')[1]*1.4,y=par('usr')[4]-(par('usr')[4]-par('usr')[3])*0.2,'Non-stationary',col='darkred')
...
points(c(median(sd.catch.st),median(sd.spn.st))~c(0.9,1.9),pch=21,bg='navy',cex=2)
lines(c(median(sd.catch.ns),median(sd.spn.ns))~c(1.1,2.1),lwd=2,col='darkred')
points(c(median(sd.catch.ns),median(sd.spn.ns))~c(1.1,2.1),pch=21,bg='darkred',cex=2)
