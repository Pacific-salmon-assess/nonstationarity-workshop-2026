...
text(x=par('usr')[2]*0.9,y=target.ER[1]*1.1,paste('Umsy=',round(umsy[1],2)),col='goldenrod',cex=0.8)
text(x=par('usr')[2]*0.9,y=target.ER[2]*1.1,paste('Umsy=',round(umsy[2],2)),col='darkred',cex=0.8)
text(x=par('usr')[2]*0.9,y=target.ER[3]*1.1,paste('Umsy=',round(umsy[3],2)),col='navy',cex=0.8)
text(x=eg.limit[1],y=0.005,paste('Smsy=',round(smsy[1],0)),col='goldenrod',cex=0.8)
text(x=eg.limit[2],y=0.005,paste('Smsy=',round(smsy[2],0)),col='darkred',cex=0.8)
text(x=eg.limit[3],y=0.005,paste('Smsy=',round(smsy[3],0)),col='navy',cex=0.8)
...
eg.limit.st=eg.scalar*mse.prod.st$smsy.est
eg.limit.ns=eg.scalar*mse.prod.ns$smsy.est
upper.tar.st=upper.tar.scalar*mse.prod.st$smsy.est
upper.tar.ns=upper.tar.scalar*mse.prod.ns$smsy.est

target.ER.st=U.scalar*mse.prod.st$umsy.est
target.ER.ns=U.scalar*mse.prod.ns$umsy.est

par(mfrow=c(2,1))
#stable Ricker model plot
plot(c(0,1)~c(0,2*max(upper.tar.st[tp])),ylab='Target harvest',xlab='Run size',type='n',main='Stable Ricker model')
lines(mse.prod.st$harvest.rate~mse.prod.st$run.size,col=adjustcolor('darkgray',alpha.f=0.5))
points(mse.prod.st$harvest.rate~mse.prod.st$run.size)
text(x=mse.prod.st$run.size,y=mse.prod.st$harvest.rate+.02,mse.prod.st$by,cex=0.5)
lines(rep(U.min,2)~c(0,max(eg.limit.st[tp])),lwd=2)
lines(c(U.min,target.ER.st[tp[1]])~c(eg.limit.st[tp[1]],upper.tar.st[tp[1]]),lwd=2,col='navy')
lines(c(U.min,target.ER.st[tp[1]])~c(eg.limit.st[tp[1]],upper.tar.st[tp[1]]),lwd=2,col='navy')
lines(rep(target.ER.st[tp[1]],2)~c(upper.tar.st[tp[1]],2*max(upper.tar.st)),lwd=2,col='navy')
lines(c(U.min,target.ER.st[tp[2]])~c(eg.limit.st[tp[2]],upper.tar.st[tp[2]]),lwd=2,col='darkred')
lines(c(U.min,target.ER.st[tp[2]])~c(eg.limit.st[tp[2]],upper.tar.st[tp[2]]),lwd=2,col='darkred')
lines(rep(target.ER.st[tp[2]],2)~c(upper.tar.st[tp[2]],2*max(upper.tar.st)),lwd=2,col='darkred')
lines(c(U.min,target.ER.st[tp[3]])~c(eg.limit.st[tp[3]],upper.tar.st[tp[3]]),lwd=2,col='goldenrod')
lines(c(U.min,target.ER.st[tp[3]])~c(eg.limit.st[tp[3]],upper.tar.st[tp[3]]),lwd=2,col='goldenrod')
lines(rep(target.ER.st[tp[3]],2)~c(upper.tar.st[tp[3]],2*max(upper.tar.st)),lwd=2,col='goldenrod')
...
text(x=par('usr')[2]*0.9,y=target.ER.st[tp[3]]*1.05,paste(round(mse.prod.st$umsy.est[tp[3]],2)),col='goldenrod',cex=0.8)
text(x=par('usr')[2]*0.9,y=target.ER.st[tp[2]]*1.05,paste(round(mse.prod.st$umsy.est[tp[2]],2)),col='darkred',cex=0.8)
text(x=par('usr')[2]*0.9,y=target.ER.st[tp[1]]*1.05,paste(round(mse.prod.st$umsy.est[tp[1]],2)),col='navy',cex=0.8)
text(x=eg.limit.st[tp[3]],y=0,paste(round(mse.prod.st$smsy.est[tp[3]],0)),col='goldenrod',cex=0.8)
text(x=eg.limit.st[tp[2]],y=-0.01,paste(round(mse.prod.st$smsy.est[tp[2]],0)),col='darkred',cex=0.8)
text(x=eg.limit.st[tp[1]],y=0.01,paste(round(mse.prod.st$smsy.est[tp[1]],0)),col='navy',cex=0.8)
points(mse.prod.st$umsy.true[tp[3]]~mse.prod.st$smsy.true[tp[3]],pch=21,cex=2,bg='goldenrod')
points(mse.prod.st$umsy.true[tp[2]]~mse.prod.st$smsy.true[tp[2]],pch=21,cex=2,bg='darkred')
points(mse.prod.st$umsy.true[tp[1]]~mse.prod.st$smsy.true[tp[1]],pch=21,cex=2,bg='navy')
points(mse.prod.st$harvest.rate~mse.prod.st$run.size)
text(x=mse.prod.st$run.size*1.02,y=mse.prod.st$harvest.rate*1.02,mse.prod.st$by,cex=0.5)

#nonstationary Ricker model plot
plot(c(0,1)~c(0,2*max(upper.tar.ns[tp])),ylab='Target harvest',xlab='Run size',type='n',main='Non-stationary Ricker model')
lines(mse.prod.ns$harvest.rate~mse.prod.ns$run.size,col=adjustcolor('darkgray',alpha.f=0.5))
points(mse.prod.ns$harvest.rate~mse.prod.ns$run.size)
text(x=mse.prod.ns$run.size,y=mse.prod.ns$harvest.rate+.02,mse.prod.ns$by,cex=0.5)
lines(rep(U.min,2)~c(0,max(eg.limit.ns[tp])),lwd=2)
lines(c(U.min,target.ER.ns[tp[1]])~c(eg.limit.ns[tp[1]],upper.tar.ns[tp[1]]),lwd=2,col='navy')
lines(c(U.min,target.ER.ns[tp[1]])~c(eg.limit.ns[tp[1]],upper.tar.ns[tp[1]]),lwd=2,col='navy')
lines(rep(target.ER.ns[tp[1]],2)~c(upper.tar.ns[tp[1]],2*max(upper.tar.ns)),lwd=2,col='navy')
lines(c(U.min,target.ER.ns[tp[2]])~c(eg.limit.ns[tp[2]],upper.tar.ns[tp[2]]),lwd=2,col='darkred')
lines(c(U.min,target.ER.ns[tp[2]])~c(eg.limit.ns[tp[2]],upper.tar.ns[tp[2]]),lwd=2,col='darkred')
lines(rep(target.ER.ns[tp[2]],2)~c(upper.tar.ns[tp[2]],2*max(upper.tar.ns)),lwd=2,col='darkred')
lines(c(U.min,target.ER.ns[tp[3]])~c(eg.limit.ns[tp[3]],upper.tar.ns[tp[3]]),lwd=2,col='goldenrod')
lines(c(U.min,target.ER.ns[tp[3]])~c(eg.limit.ns[tp[3]],upper.tar.ns[tp[3]]),lwd=2,col='goldenrod')
lines(rep(target.ER.ns[tp[3]],2)~c(upper.tar.ns[tp[3]],2*max(upper.tar.ns)),lwd=2,col='goldenrod')
text(x=par('usr')[2]*0.1,y=par('usr')[4]*0.85, paste('year', tp[3]),col='goldenrod',cex=0.8)
text(x=par('usr')[2]*0.1,y=par('usr')[4]*0.9, paste('year', tp[2]),col='darkred',cex=0.8)
text(x=par('usr')[2]*0.1,y=par('usr')[4]*0.95, paste('year', tp[1]),col='navy',cex=0.8)
text(x=par('usr')[2]*0.9,y=target.ER.ns[tp[3]]*1.05,paste(round(mse.prod.ns$umsy.est[tp[3]],2)),col='goldenrod',cex=0.8)
text(x=par('usr')[2]*0.9,y=target.ER.ns[tp[2]]*1.05,paste(round(mse.prod.ns$umsy.est[tp[2]],2)),col='darkred',cex=0.8)
text(x=par('usr')[2]*0.9,y=target.ER.ns[tp[1]]*1.05,paste(round(mse.prod.ns$umsy.est[tp[1]],2)),col='navy',cex=0.8)
text(x=eg.limit.ns[tp[3]],y=0,paste(round(mse.prod.ns$smsy.est[tp[3]],0)),col='goldenrod',cex=0.8)
text(x=eg.limit.ns[tp[2]],y=-0.01,paste(round(mse.prod.ns$smsy.est[tp[2]],0)),col='darkred',cex=0.8)
text(x=eg.limit.ns[tp[1]],y=0.01,paste(round(mse.prod.ns$smsy.est[tp[1]],0)),col='navy',cex=0.8)
points(mse.prod.ns$umsy.true[tp[3]]~mse.prod.ns$smsy.true[tp[3]],pch=21,cex=2,bg='goldenrod')
points(mse.prod.ns$umsy.true[tp[2]]~mse.prod.ns$smsy.true[tp[2]],pch=21,cex=2,bg='darkred')
points(mse.prod.ns$umsy.true[tp[1]]~mse.prod.ns$smsy.true[tp[1]],pch=21,cex=2,bg='navy')
```
...
for(i in 1:iter){
  ...
  mean(mse.prod.st$catch/max(c(mse.prod.st$catch,mse.prod.ns$catch)))
  m.catch.st[i]=mean(sims.st[[i]]$catch/max(c(sims.st[[i]]$catch,sims.ns[[i]]$catch)))
  m.catch.ns[i]=mean(sims.ns[[i]]$catch/max(c(sims.st[[i]]$catch,sims.ns[[i]]$catch)))
...
plot(c(0,1)~c(0.6,2.4),type='n',ylim=c(0,1),xaxt='n',ylab='Scaled to max. observed (C) or Proportion of years (S)',xlab='')
lines(c(median(m.catch.st)-sd(m.catch.st),median(m.catch.st)+sd(m.catch.st))~rep(0.95,2),col='navy')
lines(c(median(m.spn.st)-sd(m.spn.st),median(m.spn.st)+sd(m.spn.st))~rep(1.95,2),col='navy')
lines(c(median(m.catch.ns)-sd(m.catch.ns),median(m.catch.ns)+sd(m.catch.ns))~rep(1.05,2),col='darkred')
lines(c(median(m.spn.ns)-sd(m.spn.ns),median(m.spn.ns)+sd(m.spn.ns))~rep(2.05,2),col='darkred')
lines(c(median(m.catch.st),median(m.spn.st))~c(0.95,1.95),lwd=2,col='navy')
points(c(median(m.catch.st),median(m.spn.st))~c(0.95,1.95),pch=21,bg='navy',cex=2)
lines(c(median(m.catch.ns),median(m.spn.ns))~c(1.05,2.05),lwd=2,col='darkred')
points(c(median(m.catch.ns),median(m.spn.ns))~c(1.05,2.05),pch=21,bg='darkred',cex=2)
mtext(side=1,'Scaled Catch',at=1,line=1)
mtext(side=1,'Prop. years above EG',at=2,line=1)
text(x=par('usr')[1]*1.4,y=par('usr')[4]-(par('usr')[4]-par('usr')[3])*0.15,'Stable',col='navy')
text(x=par('usr')[1]*1.4,y=par('usr')[4]-(par('usr')[4]-par('usr')[3])*0.2,'Non-stationary',col='darkred')
