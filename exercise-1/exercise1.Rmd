...
st.fit1=ricker_TMB(data=df.st,silent=T)#note silent = T just surpresses the updates on the likelihood gradient, the default includes priors - you can set priors_flag = 0 will turn off priors on Ricker parameters
...
```
Note that the default of *ricker_TMB* applies priors on the parameters, as we generally encourage this to restrict the parameter space to areas that are biologically relevant. The $S_{max}$ prior is fairly informative, which we set as a default to a mean of half of the maximum observed spawner abundance and a standard deviation of double of that same quantity - drawn from a normal distribution. This is to prevent estimates of capacity that are widely beyond what's ever been observed (e.g. >10x the highest observed spawner estimate) and is based on an empirical ratio without any priors applied.
...
lines(pred.est1~S_p,col='darkred',lwd=2)
pred.est2=exp(st.fit2$logalpha-S_p/st.fit2$Smax)*S_p
lines(pred.est2~S_p,col='navy',lwd=2)
```
...
This nonstationary models can be in *samEst* using the *ricker_rw_TMB* (or *ricker_rw_stan*) functions. To choose between productivity or capacity changes, one must specify the time-varying parameter(*tv.par* as *a* or *b* - one can technically specify *both*, but we discourage this for reasons we will discuss).
...
In this first simulation, we will develop spawner-recruit dynamics under linear changes and see if we can successfully estimate these changes with the non-stationary (random walk) version of the Ricker model. Here in productivity:
...
This transition matrix contains the estimated probabilities of either staying ($p_{1,1}$ and $p_{2,2}$) or shifting from one state to another ($p_{1,2}$ and $p_{2,1}$) for each time-step in a time-series. In practice, this results in some inertia or latency that expects system dynamics to stay in a given regime for several years at a time before eventually transitioning to the other state - recreating the sort of regime shift dynamics we would expect to occur in a natural system.
...
df.reg.prod=salmon_sim.tv(log.a0=log.a0,p.change=p.change,smax0=smax0,sigma=sigma,N=N,par='a',form='regime',reg.length=8)
...
lines(hmm.fit.prod$logalpha[hmm.fit.prod$regime],lwd=2,col='darkred')
...
lines(as.numeric(hmm.fit.prod$logalpha.t),lwd=2,col='darkred')
lines(rw.fit.reg$logalpha,lwd=2,col='navy')
```
...
df.reg.b=salmon_sim.tv(log.a0=log.a0,p.change=p.change,p.change2=p.change2,smax0=smax0,sigma=sigma,N=N,par='both',form='regime',reg.length=8)
...
```{r, prod cap regime true vs estimated,fig.height=8,fig.width=6}
par(mfrow=c(2,1))
plot(df.reg.b$loga.t,type='l',lwd=2,ylim=c(min(c(df.reg.b$loga.t,hmm.fit.b$logalpha)),max(c(df.reg.b$loga.t,hmm.fit.b$logalpha))))
lines(hmm.fit.b$logalpha[hmm.fit.b$regime],lwd=2,col='darkred')
plot(df.reg.b$smax.t,type='l',lwd=2,ylim=c(min(c(df.reg.b$smax.t,hmm.fit.b$Smax)),max(c(df.reg.b$smax.t,hmm.fit.b$Smax))))
