...
Another way to visualize this is to multiply the probability of each regime by its expected parameter - this (often) ends up being conceptually similar to the random walk:
...
plot(df.reg.prod$loga.t,type='l',lwd=2,ylim=c(min(c(df.reg.prod$loga.t,hmm.fit.prod$logalpha.t,rw.fit.reg$logalpha)),max(c(df.reg.prod$loga.t,hmm.fit.prod$logalpha.t,rw.fit.reg$logalpha))))
