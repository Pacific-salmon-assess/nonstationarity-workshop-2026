...
Note that if you are curious about uncertainty in the parameter estimates these can be examined in the output by calling `sd_report`.

```{r, sd report}
st.fit1$sd_report
```
For *Stan* versions you can extract the full posteriors for any parameter by calling `full_posterior`.
...
The following exercises are intended to give participants a broad understanding of how stable and non-stationary spawner-recruit dynamics influence estimates of biological reference points - and the consequences of these different estimation approaches for stock assessment and management scenarios. We encourage you to modify the R code chunks to experiment with each section to explore the mechanics of the simulations and model testing.
...
Additionaly, we have tweaks to the exact details of how the harvest control rule relates to the reference points: `U.min` the minimum harvest rate (below the escapement goal), `eg.scalar` - the escapement goal as a fraction of $S_{msy}$ (e.g. 1 = $S_{msy}$), `upper.tar.scalar` - the upper run size where the maximum harvest rate is achieved as a fraction of $S_{msy}$, and `U.scalar` - the maximum harvest rate as a fraction of $U_{msy}$ (e.g. 1 = $U_{msy}$).
...
We can visualize a harvest control rule that is fairly common - with a trigger for reduced harvest based on some fraction of $S_{msy}$ representing the escapement goal (below which, we may expect only a limited harvest for e.g. incidental, recreational, or Indigeneous fishing rights catch), with a harvest target beyond that limit that ramps up toward some fraction of $U_{msy}$ based on the run size - here plotted for 3 different $log(\alpha)$ estimates that may represent change at various time-points (e.g. in this case a decline in productivity).
...
We can also visualize how this results in different harvest control rules at different time-points in the simulation where the reference points are estimated (indicated by the coloured lines) relative to their true simulation reference points (indicated by the ***solid, coloured*** points) in the simulation:
