...
```{r, static ricker simulation,fig.width=7,fig.height=9}
#Ricker parameters
...
Let's see how 1-year autocorrelation affects parameter estimates. To account for autocorrelation in our estimation models, we set the `ac` option in `ricker_TMB()` or `ricker_stan()` to `TRUE`.
...
Typically, the autocorrelated model is what we would consider a 'baseline' model for a stable system. It's pretty common that there is some degree of autocorrelation (typically a positive value for $\rho$) in year-to-year recruitment (with the exception of Pink). The autocorrelation parameter is also often indicative as a statistical signal of long-term change in system dynamics, which is what we will explore in the next sections.
...
We will test how well these models can actually capture trends in these Ricker parameters using another simulation function made for this workshop - here we can change either intrinsic productivity or capacity to time-varying by specifying `tv.par = a` or `tv.par = b` in this function.
...
```{r, rw plot smax,fig.width=7,fig.height=9}
...
In real time-series of salmon spawner-recruit dynamics, we won't know which parameter is actually nonstationary (if any). We can fit models that are mismatched to their simulation properties (e.g. fit a non-stationary productivity to a scenario where $S_{max}$ is changing) to see what the mis-specified models estimate:

```{r, tv est - linear crossed,fig.width=7,fig.height=9}
...
Which can be thought of as the harvest (or exploitation) rate that equivalently produces maximum sustainable yield over the long-term.
...
```
Not that when $S_{max}$ varies, but not $log(\alpha)$, that $U_{msy}$ will not change (as per the formula above).


Let's use a familiar scenario where productivity ($log(\alpha)$) or capacity ($S_{max}$) is declining - we will calculate both the true reference points (based on the underlying simulated parameters, black) and the estimates (red) based on the random walk model. Feel free to switch this function to `rw` or `regime` to test out other forms.
...
As productivity ($log(\alpha)$) declines so too does $S_{msy}$ and $U_{msy}$ (although not linearly, due to their log-scaling). Similarly, if $S_{max}$ changes instead, so too does $S_{msy}$ - but not $U_{msy}$, which is only based on $log(\alpha)$.

# Stable and non-stationary harvest control rules

Imagine then how harvest in a salmon fishery may be set based on these reference points. There are a diversity of different 'harvest control rules' that are set for various salmon fisheries throughout the North Pacific - one commonality is that there is typically a target for exploitation rates based on the returning run size in a given year, as well as a lower limit around which harvest is curtailed to ensure enough fish return to spawn in their natal watersheds to maintain recruitment of future generations (i.e. an escapement goal). Typically, these two quantities - the target rate and escapement goal - will be influenced by estimates of stock-specific reference points: $U_{msy}$ and $S_{msy}$, respectively.

If we then think about how non-stationary versions of these reference points can be translated into a harvest control rule - the result is that these reference points then become 'moving targets' based on estimates from the statistical information in the spawner-recruit time-series.

We can visualize a harvest control rule that is fairly common - with a trigger for reduced harvest based on some fraction of $S_{msy}$ representing the escapement goal (below which, we may expect only a limited harvest for e.g. incidental, recreational, or Indigenous fishing rights catch), with a harvest target beyond that limit that ramps up toward some fraction of $U_{msy}$ based on the run size - here plotted for 3 different $log(\alpha)$ estimates that may represent change at various time-points (e.g. in this case a decline in productivity).

Key parameters include our standard Ricker spawner-recruit parameters, that influence the reference points that feed into the harvest control rule through the estimated reference points.

Additionally, we have tweaks to the exact details of how the harvest control rule relates to the reference points: `U.min` the minimum harvest rate (below the escapement goal - 0.05 = 5%), `eg.scalar` - the escapement goal as a fraction of $S_{msy}$ (e.g. 1 = $S_{msy}$), `upper.tar.scalar` - the upper run size where the maximum harvest rate is achieved (also as a fraction of $S_{msy}$), and `U.scalar` - the maximum harvest rate as a fraction of $U_{msy}$ (e.g. 1 = $U_{msy}$).
...
Note - that changes to $S_{max}$ will change the escapement goal and shift when exploitation begins to increase with run size, but not the harvest target based on $U_{msy}$.

From the above we can see 2 main themes: that lower estimates of $S_{msy}$ will result in lowering the escapement goal trigger to lower levels of abundance, while lower estimates of $U_{msy}$ will reduce the harvest target at a given run size above the escapement goal.
...
Note that there is a countervailing effect in how changing reference points work in the harvest control rule with an escapement goal trigger and lower limit harvest rate. While $U_{msy}$ will decrease with lower productivity - resulting in lower harvest targets, $S_{msy}$ also decreases at lower productivity - resulting in a lower run size trigger to commence higher harvest targets.
...
Try altering the simulation parameters to other magnitudes or forms of parameter change, and assessment types, to explore how these trade-offs may alter according to the specific simulation scenarios - you can do this either in the previous section to look at individual outcomes or below to get an understanding of the mean and variability in outcomes under a set of scenario conditions.
...
We can explore one last form of harvest control rule with stable and non-stationary assessments, and that is combining estimates from both model types. As highlighted previously, the reduction in $U_{msy}$ with productivity tend to be conservative under scenarios of declining productivity, but the concomitant reduction in $S_{msy}$ is the opposite - it encourages lowering the escapement goals to permit more fishing at lower levels of return abundances. We can explore then how using a 'mixed' harvest control rule - where the harvest reference point is set with a non-stationary model, and the abundance-based reference point (escapement goal) is set with the stable model - may perform under this same scenario. 
